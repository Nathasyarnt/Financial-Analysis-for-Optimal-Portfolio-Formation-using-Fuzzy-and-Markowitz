...
library(psych)
#Fuzzy C-Means
res.fcm <- fcm(dataasuransi, centers=3, dmetric="euclidean")
...
cluster::clusplot(scale(dataasuransi), res.fcm3$cluster,
                  main = "Asuransi Cmeans 3 Cluster",
...
icdrate(dataasuransi1,length(dataasuransi1),3)

#==================GUSTAFSON KESSEL CLUSTERING=========================
#Gustafson Kessel
cl.gk=gk(dataasuransi, 3, m=3, dmetric="euclidean")
cl.gk
#Fuzzy Membership Matrix
...
cluster::clusplot(scale(dataasuransi), cl.gk$cluster,
                  main = "Asuransi Gustaf 3 Cluster",
                  color=TRUE, labels = 2, lines = 2, cex=1)
#icdrate and Pseudo-F
...
icdrate(dataasuransi2,length(dataasuransi2),3)
